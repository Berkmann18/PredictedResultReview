const { difference, diff, fileToArr, strToMtx } = require('../src/diff')

/* global test:false, expect:false */

test('difference', () => {
  let a = [0, 1, 2, 4]; let b = [0, 1, 2, 3]
...
test('string->matrix', () => {
  expect(strToMtx('Lorem Ipsum')).toStrictEqual([['Lorem Ipsum']])
  expect(strToMtx('Lorem,Ipsum')).toStrictEqual([['Lorem', 'Ipsum']])
  expect(strToMtx('Lorem Ipsum', ' ')).toStrictEqual([['Lorem', 'Ipsum']])
  expect(strToMtx('Lorem Ipsum')).toStrictEqual([['Lorem Ipsum']])
  expect(strToMtx('Lorem Ipsum\nDolore Sit')).toStrictEqual([['Lorem Ipsum'], ['Dolore Sit']])
  expect(strToMtx('Lorem Ipsum\nDolore Sit', ' ')).toStrictEqual([['Lorem', 'Ipsum'], ['Dolore', 'Sit']])
  expect(strToMtx('Lorem Ipsum;Dolore Sit', ' ', ';')).toStrictEqual([['Lorem', 'Ipsum'], ['Dolore', 'Sit']])
})
